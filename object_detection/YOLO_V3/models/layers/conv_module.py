...
class Convolutional(nn.Module):
    def __init__(self, filters_in = None , filters_out= None , kernel_size = 1, stride=0, padding=1, batch_norm=None, activate=None):
        super(Convolutional, self).__init__()
        self.batch_norm = batch_norm
        self.activate = activate
        self.__conv = nn.Conv2d(in_channels=filters_in, out_channels=filters_out, kernel_size=kernel_size, stride=stride,
                                padding=padding, bias=not batch_norm)
        if batch_norm:
            assert batch_norm in norm_name.keys(), "Norm not define in given list"
            if batch_norm == "bn":
                self.__norm = norm_name[batch_norm](num_features=filters_out)
        if activate:
